...
#import "LYouLoadView.h"

static LYouLoadView *activiyView;

@interface LYouLoadView ()
...
+ (void)show {
    ...
    dispatch_once(&onceToken, ^{
        if (activiyView == nil) {
            ...
            activiyView = [[LYouLoadView alloc] initWithFrame:rect];
...
- (UIView *)contentView {
    if (_contentView == nil) {
        ...
        contentView.backgroundColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.2];
...
- (UILabel *)label {
    if (_label == nil) {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.reaplicator.frame)+25, CGRectGetWidth(self.contentView.frame), 20)];
        label.textAlignment = NSTextAlignmentCenter;
        label.textColor = UIBtnTextBlueTheme;
        label.text = @"加载中...";
...
- (CAReplicatorLayer *)reaplicator{
    if (_reaplicator == nil) {
        ...
        repelicator.bounds = CGRectMake(0, 0, 10, 10);
        ...
        layer.backgroundColor = UIBtnTextBlueTheme.CGColor;
